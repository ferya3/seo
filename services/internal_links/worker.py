"""Internal links service — event-bus worker.

Consumes `links.analysis_requested` and runs the same `run_analysis` the HTTP
route uses.
...
def handle(envelope: Envelope) -> None:
    ...
    try:
        validate_event("links.analysis_requested", envelope.payload)
    except ContractError as exc:
        # Dead-letter: a malformed payload never becomes valid on redelivery.
        raise ValueError(f"invalid links.analysis_requested: {exc}") from exc
...
def main() -> None:
    ...
    Consumer(QUEUE, ["links.analysis_requested"], handle).run()
